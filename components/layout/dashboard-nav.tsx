...
const NavItemContent = React.memo(({ item, isMinimized, isExpanded, path }: { item: NavItem; isMinimized: boolean; isExpanded: boolean; path: string }) => {
  const Icon = item.icon ? Icons[item.icon] : Icons.logo;
  const hasChildren = item.children && item.children.length > 0;

  return (
    <div className={cn("flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground", path === item.href ? "bg-accent" : "transparent", item.disabled && "cursor-not-allowed opacity-80")}>
...
const NavItemLink = React.memo(({ item, onClick, children }: { item: NavItem; onClick: () => void; children: React.ReactNode }) => (
  <Link href={item.disabled ? "/" : item.href || "#"} className={cn("block", item.disabled && "cursor-not-allowed opacity-80")} onClick={onClick}>
    {children}
  </Link>
));
    ...
    (item: NavItem, depth = 0) => {
      ...
      return (
        <div key={item.title}>
          ...
          {hasChildren && !isMinimized && isExpanded && <div className="ml-4 mt-1 space-y-1">{item.children && item.children.map((child) => renderNavItem(child, depth + 1))}</div>}
        </div>
      );
    },

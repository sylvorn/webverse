  ...
  return (
    <div className="w-full bg-neutral-950 font-sans md:px-10" ref={containerRef}>
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        <h2 className="text-lg md:text-4xl mb-4 text-white max-w-4xl">{data.name}</h2>
        <p className="text-neutral-300 text-sm md:text-base max-w-sm">{data.description}</p>
      </div>
        ...
        {data.features.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:pt-40 md:gap-10">
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              ...
              {parse(item.content as string, options)}
            </div>
          </div>
        ))}
        <div

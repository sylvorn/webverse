...
export const InfiniteMovingLogos = ({
  ...
  //       {items.map((item, idx) => (
  //         <li className="w-[350px] max-w-full relative items-center flex-shrink-0 px-8 py-6 md:w-[450px]" key={idx}>
  //           <img src={item.imgUrl} alt="Image" className="" />
  //         </li>
  //       ))}
  //     </ul>
  //   </div>
  ...
  //     <ul ref={scrollerRef} className={cn("flex min-w-full shrink-0 gap-4 py-4 w-max flex-nowrap", start && "animate-scroll", pauseOnHover && "hover:[animation-play-state:paused]")}>
  //       {items.map((item, idx) => (
  //         <li className="w-[350px] md:w-[450px] max-w-full relative flex items-center justify-center flex-shrink-0 px-8 py-6" key={idx}>
  //           <img src={item.imgUrl} alt="Image" className="max-w-full max-h-full" />
  //         </li>
  //       ))}
  //     </ul>
  //   </div>
        ...
        {items.map((item, idx) => (
          <li className="w-[350px] md:w-[450px] max-w-full relative flex items-center justify-center flex-shrink-0 px-8 py-6" key={idx}>
            <Link href={item.webUrl} target="_blank" rel="noopener noreferrer">
              <img src={item.imgUrl} alt="Image" className="max-w-[200px] max-h-[200px] object-contain" />
            </Link>
          </li>
        ))}
      </ul>
